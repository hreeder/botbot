...
@bot.webhook("/gogs")
class GogsHandler(RequestHandler):
    def initialize(self):
        self.prefix = "{}[Gogs]{}".format(Format.GREEN, Format.RESET)
        self.bot = self.application._ctx

    @coroutine
    def post(self):
        hook_type = self.request.headers['X-Gogs-Event']
    ...
    def handle_push(self, target_channel, body):
        who = body['commits'][0]['committer']['name']
    ...
    def handle_pull_request(self, target_channel, body):
        ...
        elif state == "label_updated":
            message = "{} {}{}{} - {}{}{} updated labels on {}".format(
                self.prefix,
                Format.ORANGE, repo, Format.RESET,
                Format.BLUE, body['sender']['login'], Format.RESET,
                pr_info
            )
        elif state == "label_cleared":
            message = "{} {}{}{} - {}{}{} cleared labels on {}".format(
                self.prefix,
                Format.ORANGE, repo, Format.RESET,
                Format.BLUE, body['sender']['login'], Format.RESET,
                pr_info
