...
def slackwho(bot, channel, sender, args):
    ...
    while users:
        ...
        try:
            ...
            pass
        output = "Slack Users: {} ".format(", ".join(outlist))
...
def slackwhois(bot, channel, sender, args):
    ...
    slacker = Slacker(bot.config['Slack']['api_key'])
    if not args:
        bot.message(channel, "{}: Please supply a user to look up".format(sender))
        return
    users_response = slacker.users.list(presence=True)
    users = {user['name']: user for user in users_response.body['members']}

    if args[0] not in users:
        bot.message(channel, "{}: {} was not found in the slack team".format(sender, args[0]))
        return
    ...
    if user['profile']['real_name']:
        name_str += " ({})".format(user['profile']['real_name'])

    bot.message(channel, "Slack User: {}, Presence: {}".format(name_str, user['presence']))
